...
#define homePin 4
#define awayPin 5
#define conveyorMotorPin 6
#define trainSensorPin 7
#define bucketFullSensorPin 8
#define hopperBusyPin 9
...
// Define Analog pins
#define delayAnalog A0
#define speedAnalog A1
#define lengthAnalog A2
...
#define DEFAULT_HOME_OPEN true 

// Specify if Away switch is Normally Open or Closed
#define DEFAULT_AWAY_OPEN true 

// Specify if Hopper Triger is Active High or Low 
#define TRIGGER_ACTIVE_HIGH false 

// Specify if Train Sensor is Active High or Low 
#define TRAINSENSOR_ACTIVE_HIGH false
...
bool homeSwitchNO = DEFAULT_HOME_OPEN;

// Conveyor will run for 60 seconds
#define CONVEYOR_RUN_TIME 60000L
unsigned long stopConveyorTime = 0L;
...
void setup() 
{
  ...
  pinMode(trainSensorPin, INPUT_PULLUP);
  pinMode(bucketFullSensorPin, INPUT_PULLUP);
...
IsTriggerActive()
{
  ...
  {
    // It is active high
    if (digitalRead(bucketFullSensorPin) == HIGH)
    {
      return(true);
    }
    else
  ...
  {
    // It is active low
    if (digitalRead(bucketFullSensorPin) == LOW)
    {
      return(true);
    }
    else
...
{
  // Check to see if its active high or low
  if (TRAINSENSOR_ACTIVE_HIGH)
  {
    // It is active high
    if (digitalRead(trainSensorPin) == HIGH)
    {
      return(true);
    }
    else
  ...
  {
    // It is active low
    if (digitalRead(trainSensorPin) == LOW)
    {
      return(true);
    }
    else
...
WaitForHopperTrigger()
{
  ...
  {
    // Check the train sensor
    if (IsTrainSensed())
    {
      // Yes, start the conveyor and start timer
      Serial.print(F("WaitForHopperTrigger: Train Sensed, Starting Conveyor.\n"));
      
      digitalWrite(conveyorMotorPin, LOW);
      stopConveyorTime = millis() + CONVEYOR_RUN_TIME;
    }

    // Check conveyor timer
    if (stopConveyorTime > 0L)
    {
      if (millis() > stopConveyorTime)
      {
        Serial.print(F("WaitForHopperTrigger: Conveyor Timer expired, Stopping Conveyor.\n"));
        
        digitalWrite(conveyorMotorPin, HIGH);
        stopConveyorTime = 0L;
      }
    }
    // Sleep for 1/100 second if not
    delay(10);
    --count;
    ...
    if (count < 0)
    {
      ...
      // Set delay time to govern speed of hopper (Speed Trimmer)
      ...
      // Set the time to wait at the top before returning. (Delay Trimmer)
      // Bigger than int values so work it out long hand or it doesnt work.
      longScratch1 = MAX_WAIT - MIN_WAIT;
      longScratch2 = longScratch1 * analogRead(delayAnalog);
      longScratch3 = longScratch2 / 1024L;
